System.register(['lib/DataSet.js', 'lib/util.js', 'etc/lzma.js', 'etc/pako.min.js'], function (_export, _context) {
  "use strict";
  ...
  return {
    setters: [function (_libDataSetJs) {
      ...
      LZMA = _etcLzmaJs.default;
    }, function (_etcPakoMinJs) {
      pako = _etcPakoMinJs.default;
    }],
    execute: function () {
      ...
      const DataSetIO = {
        ...
        checkCompression(compression) {
          ...
          if (!this.compressionNames.includes(compression)) util.error(`DataSetIO: illegal compression ${ compression }`);
        },
        ...
        toJson(dataset, compression = 'none', level = 9) {
          ...
          if (compression === 'lzma') return this.lzmaCompressPromise(dataset, level);
          if (compression === 'none') return this.toJsonString(dataset, dataset.data, compression, level);

          let uint8array = util.arrayToBuffer(dataset.data);
          if (compression === 'zip') uint8array = pako.deflate(uint8array, { level });
        ...
        toDataSet(json) {
          ...
          if (compression === 'lzma') return this.lzmaDecompressPromise(jsonObj);
          if (compression === 'none') return new DataSet(width, height, util.convertArray(data, window[datatype]));

          let uint8array = util.base64ToBuffer(data);
          if (compression === 'zip') uint8array = pako.inflate(uint8array);
        ...
        toJsonObject(dataset, data = dataset.data, compression = 'none', level = 9) {
          data = compression === 'none' ? util.convertArray(data, Array) : util.bufferToBase64(data);
          const jsonObj = {
            width: dataset.width,
            height: dataset.height,
        ...
        isLZMA(json) {
          return json.search(/compression":"lzma/);
        },
