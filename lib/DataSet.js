    ...
    execute: function () {
      ...
      class DataSet {
        ...
        static concat(array1, array2) {
          ...
          if (Type === Array) return array1.concat(DataSet.typedToArray(array2));
        ...
        resample(width, height, useNearest = true) {
          ...
          const ds = new DataSet(width, height, new this.data.constructor(width * height));
          // const data = ds.data
        ...
        col(x) {
          ...
          for (let i = 0; i < h; i++) colData.push(data[x + i * w]);
          return colData;
        }
        ...
        concatEast(dataset) {
          const [w, h] = [this.width, this.height];
          if (h !== dataset.height) throw `concatEast: heights not equal ${ h }, ${ dataset.height }`;
          let newData = [];
          for (let i = 0; i < h; i++) newData = newData.concat(this.row(i).concat(dataset.row(i)));
        ...
        convertType(Type) {
          ...
          this.data = Type === Array ? DataSet.typedToArray(this.data, Type) : DataSet.arrayToTyped(this.data, Type);
        }
