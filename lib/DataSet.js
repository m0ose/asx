  ...
  return {
    ...
    execute: function () {
      ...
      class DataSet {
        ...
        checkXY(x, y) {
          if (!this.inBounds(x, y)) u.error(`DataSet.checkXY: x,y out of range: ${ x }, ${ y }`);
        }

        inBounds(x, y) {
          if (!(u.between(x, 0, this.width - 1) && u.between(y, 0, this.height - 1))) return false;
          return true;
        }
        ...
        getXY(x, y) {
          return this.data[this.toIndex(Math.floor(x), Math.floor(y))];
        }

        // Set the data value at x,y to num
        setXY(x, y, num) {
          this.data[this.toIndex(Math.floor(x), Math.floor(y))] = num;
        }
