  ...
  return {
    ...
    execute: function () {
      ...
      class DataSet {
        ...
        checkXY(x, y) {
          if (!this.inBounds(x, y)) util.error(`DataSet.checkXY: x,y out of range: ${ x }, ${ y }`);
        }
        ...
        resample(width, height, useNearest = true, Type = Array) {
          ...
          for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) ds.setXY(x, y, this.sample(x * xScale, y * yScale, useNearest));
          return ds;
        }
        ...
        subset(x, y, width, height) {
          if (x + width > this.width || y + height > this.height) util.error('DataSet.subSet: params out of range');
        ...
        col(x) {
          ...
          if (x >= w) util.error(`col: x out of range width: ${ w } x: ${ x }`);
        ...
        row(y) {
          ...
          if (y >= h) util.error(`row: y out of range height: ${ h } x: ${ y }`);
        ...
        concatEast(ds) {
          ...
          if (h !== h1) util.error(`concatEast: heights not equal ${ h }, ${ h1 }`);
          ...
          for (let y = 0; y < w1; y++) ds1.setXY(x + w, y, ds.getXY(x, y));
          return ds1;
        }
        ...
        concatSouth(dataset) {
          const [w, h, data] = [this.width, this.height, this.data];
          if (w !== dataset.width) util.error(`concatSouth: widths not equal ${ w }, ${ dataset.width }`);
        ...
        neighborhood(x, y, array = []) {
          ...
          const clampNeeded = x === 0 || x === this.width - 1 || y === 0 || y === this.height - 1;
        ...
        convolve(kernel, factor = 1, crop = false) {
          ...
          ? [1, 1, this.height - 1, this.width - 1] : [0, 0, this.height, this.width];
        ...
        toContext(normalize = false, gray = false, alpha = 255) {
          ...
          let idata;
          if (normalize) {
            idata = gray ? util.normalize8(data) : util.normalizeInt(data, 0, Math.pow(2, 24) - 1);
          } else {
        ...
        equals(dataset) {
          return this.width === dataset.width && this.height === dataset.height && util.arraysEqual(this.data, dataset.data);
        }
      }
