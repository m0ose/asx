    ...
    execute: function () {
      ...
      const glx = {
        ...
        addDebugWrappers(gl, webglDebug, fcnsToo = false) {
          if (fcnsToo) return webglDebug.makeDebugContext(gl, null, (functionName, args) => {
            args = webglDebug.glFunctionArgsToString(functionName, args);
            console.log(`gl.${ functionName }(${ args })`);
          });else return webglDebug.makeDebugContext(gl);
        },
        ...
        createShader(gl, shaderScript, type) {
          ...
          if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) this.error(gl.getShaderInfoLog(shader));
        ...
        createProgram(gl, vShaderScript, fShaderScript) {
          ...
          if (!gl.getProgramParameter(program, gl.LINK_STATUS)) this.error('Could not initialise shaders');
        ...
        initActiveInfo(gl, program, index, isAttrib) {
          const aInfo = isAttrib ? gl.getActiveAttrib(program, index) : gl.getActiveUniform(program, index);
          const info = { name: aInfo.name, type: aInfo.type, size: aInfo.size };
          info.location = isAttrib ? gl.getAttribLocation(program, info.name) : gl.getUniformLocation(program, info.name);
        ...
        initBuffers(gl, program, elementArrayName = null) {
          ...
          util.forEach(program.attribInfo, (aInfo, name) => {
            ...
            gl.vertexAttribPointer(location, components, gl[componentType], false, 0, 0);
