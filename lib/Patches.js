System.register(['./util.js', './AgentSet.js', './DataSet.js'], function (_export, _context) {
  "use strict";

  var util, AgentSet, DataSet;
  return {
    ...
    execute: function () {
      ...
      class Patches extends AgentSet {
        ...
        nighbors4Offsets(x, y) {
          ...
          return this.neighborsOffsets(x, y).filter(n => [1, -1, numX, -numX].indexOf(n) >= 0);
        }
        // Return my 8 patch neighbors
        neighbors(patch) {
          const { id, x, y } = patch;
          const offsets = this.neighborsOffsets(x, y);
          offsets.forEach((o, i, a) => {
            a[i] = this[o + id];
          });
        ...
        inCone(p, radius, width, angle, meToo = true) {
          const pset = this.inSquare(p, radius, meToo);
          return pset.filter(p1 => util.inCone(radius, width, angle, p.x, p.y, p1.x, p1.y) || meToo && p === p1);
        }
        ...
        draw(ctx = this.model.contexts.patches) {
          ...
          if (!pixels.are1x1) util.fillCtxWithImage(ctx, pixels.ctx.canvas);
          for (const i in this.labels) {
            // `for .. in`: skips sparse array gaps.
        ...
        importDrawing(imageSrc) {
          util.imagePromise(imageSrc).then(img => this.installDrawing(img));
        }
        ...
        importDataSet(dataSet, patchVar, useNearest = false) {
          ...
          for (const patch of this) patch[patchVar] = dataset.data[patch.id];
        }
        ...
        patchXY(x, y) {
          return this[this.patchXYToIndex(x, y)];
        }
        ...
        setLabel(patch, label) {
          if (label == null) // null or undefined
            delete this.labels[patch.id];else this.labels[patch.id] = label;
        }
        getLabel(patch) {
        ...
        diffuseN(n, v, rate, colorMap = null, min = 0, max = 1) {
          ...
          if (this[0]._diffuseNext === undefined) for (const p of this) p._diffuseNext = 0;
          ...
          for (const p of this) {
            p[v] = p._diffuseNext;
            p._diffuseNext = 0;
            if (colorMap) p.setColor(colorMap.scaleColor(p[v], min, max));
          }
        }
