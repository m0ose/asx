    ...
    execute: function () {
      ...
      const util = {
        ...
        bufferToBase64(uint8Array) {
          const binstr = Array.prototype.map.call(uint8Array, ch => String.fromCharCode(ch) // note uint8Array.map fails due to char data
          ).join('');
          return btoa(binstr);
        },
        ...
        imageToPixels(img, flipY = false) {
          ...
          if (status !== gl.FRAMEBUFFER_COMPLETE) this.error(`imageToPixels: status not FRAMEBUFFER_COMPLETE: ${ status }`);
