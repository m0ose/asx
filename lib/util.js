    ...
    execute: function () {
      ...
      const util = {
        ...
        bufferToBase64(uint8Array) {
          const binstr = Array.prototype.map.call(uint8Array, ch => String.fromCharCode(ch)).join('');
          return btoa(binstr);
        },
        ...
        bufferToByteString(uint8Array, chunkSize = 32768) {
          // 2**15
        ...
        distanceSq: (x, y, x0 = 0, y0 = 0) => (x - x0) * (x - x0) + (y - y0) * (y - y0),
        ...
        inCone(radius, width, angle, x1, y1, x2, y2) {
          if (radius * radius < this.distanceSq(x1, y1, x2, y2)) return false;
        ...
        repeat(n, f, o) {
          for (let i = 0; i < n; i++) f(i, o);return o;
        },
        ...
        keyForValue(obj, value) {
          for (const key in obj) if (obj[key] === value) //  gl problems: && obj.hasOwnProperty(key)
            return key;
          return null;
        },
        ...
        forEach(arrayOrObj, fcn) {
          if (arrayOrObj.slice) // typed & std arrays
            for (let i = 0, len = arrayOrObj.length; i < len; i++) fcn(arrayOrObj[i], i, arrayOrObj);else // obj
        ...
        waitOn(done, f, ms = 10) {
          if (done()) f();else setTimeout(() => {
            this.waitOn(done, f, ms);
          }, ms);
        },
        ...
        runGenerator(it, callback = lastVal => {}) {
          it = util.typeOf(it) === 'generator' ? it : it();(function iterate(val) {
            // asynchronously iterate over generator
            const ret = it.next(val);
            if (!ret.done) {
                // wait on promise, `then` calls iterate w/ a value
                if (ret.value.then) ret.value.then(iterate); // iterate takes the promise's value
                else // avoid synchronous recursion
                  setTimeout(() => iterate(ret.value), 0);
            } else callback(ret.value);
          })();
        },
        ...
        runAsyncFcn(fcn, thenFcn) {
          const startup = fcn();
          if (util.typeOf(startup) === 'generator') util.runGenerator(startup, thenFcn);else if (util.typeOf(startup) === 'promise') startup.then(thenFcn);else thenFcn();
        },
        ...
        setCtxSmoothing(ctx, smoothing) {
          ...
          for (const name of aliases) if (ctx[name]) return ctx[name] = smoothing; // lets hope the first one works. Sheesh!
        },
        ...
        imageToBytes(img, flipY = false, imgFormat = 'RGBA') {
          ...
          if (status !== gl.FRAMEBUFFER_COMPLETE) this.error(`imageToBytes: status not FRAMEBUFFER_COMPLETE: ${ status }`);
