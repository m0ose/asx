System.register(['lib/util.js', 'lib/Model.js'], function (_export, _context) {
  "use strict";
      ...
      class AsyncTest extends Model {
        *generatorStartup() {
        ...
        startup() {
          ...
          if (startupWithPromise) return this.promiseStartup();else return this.generatorStartup();
        }
        setup() {
        ...
        step() {
          ...
          if (this.anim.ticks === 10) {
            console.log('stopping');this.stop();
          }
        }
      }
      const model = new AsyncTest('layers').start();
      util.toWindow({ model });
      // model.start()
    }
  };
