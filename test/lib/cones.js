    ...
    execute: function () {
      ...
      class ConeModel extends Model {
        setup() {
          ...
          this.coneColors = util.repeat(population, (i, a) => {
            a[i] = cmap.randomColor();
          }, []);
          this.headings = util.repeat(population, (i, a) => {
            a[i] = util.randomInt(360);
          }, []);
        }
        step() {
          const { radius, width, patches, headings, turtles } = this;
          patches.forEach(p => {
            p.color = this.backgroundColor;
          });
          turtles.forEach((t, i) => {
            const heading = headings[i];
            const pset = this.useInRadius ? patches.inRadius(t, radius) : patches.inCone(t, radius, width, util.angle(headings[i]));
            pset.forEach(p => {
              p.color = this.coneColors[i];
            });
