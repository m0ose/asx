System.register(['lib/util.js', 'lib/Color.js', 'lib/ColorMap.js', 'lib/Model.js', 'etc/three.min.js'], function (_export, _context) {
  "use strict";

  var util, Color, ColorMap, Model, THREE;
  return {
    ...
    execute: function () {
      ...
      const modules = { Color, ColorMap, Model, util, THREE }; // Import the lib/ mmodules via relative paths
      ...
      class FireModel extends Model {
        ...
        setup() {
          ...
          for (const p of this.patches) {
            if (p.x === this.world.minX) this.ignight(p);else if (util.randomInt(100) < this.density) p.color = this.treeColor;else p.color = this.dirtColor;
          }

          this.burnedTrees = 0;
          this.initialTrees = this.patches.filter(p => p.color.equals(this.treeColor)).length;
        }
        ...
        step() {
          ...
          for (const p of this.fires) {
            for (const n of p.neighbors4) if (this.isTree(n)) this.ignight(n);
        ...
        isTree(p) {
          return p.color.equals(this.treeColor);
        }
        ...
        fadeEmbers() {
          for (const p of this.embers) {
            ...
            if (ix === this.fireColorMap.length - 1) this.patches.setBreed(p); // sorta like die, removes from breed.
            else p.color = this.fireColorMap[ix + 1];
          }
        }
      }
